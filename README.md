...
I've found one of the most satisfying experiences when writing code is building something that's scalable. By focussing on regular refactors after passing tests I've managed to build application that can grow comfortably. A good example is [bestB4](Github Link), where I made the decision to use Redux to manage our state - allowing us to later create components to call external API's with data in our Redux state.

#### Following Agile Practices
Makers is fantastic at encouraging you to follow agile practices. We consistently worked on the basis of two-day sprints - giving the opportunity to reflect on the code we'd created and move forward based on what we do know rather that what we don't. During our two weeks creating an [Acebook](https://github.com/mackacavs/acebook-agil/) clone, we had continuous two-day sprints to help us to continue to build and grow the application's MVP efficiently
